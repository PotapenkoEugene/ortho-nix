...
Usage:
    uv run --no-project --with playwright python ~/.claude/skills/excalidraw-diagram/references/render_excalidraw.py <path-to-file.excalidraw> [--output path.png]
...
def render(
    ...
    try:
        ...
        print("Run: uv run --no-project --with playwright python -m playwright install chromium", file=sys.stderr)
        sys.exit(1)
