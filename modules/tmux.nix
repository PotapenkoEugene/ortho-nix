  ...
  programs.tmux = {
    ...
    extraConfig =
      ''
        ...
        # Pane management
        bind | split-window
        bind s display-popup -w 80% -h 60% -E "bash $HOME/.config/home-manager/scripts/tmux-recency-source.sh | tv --ansi --no-preview | awk -F'\\t' '{print $NF}' | xargs -r tmux switch-client -t"
